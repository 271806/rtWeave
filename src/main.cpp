...
void banner() {
    ...
    if (!igl::readOBJ(obj_file_path_tree, V_tree, F_tree)) {
        std::cerr << "Failed to load tree OBJ file." << std::endl;
        return;
    }
    ...
    if (!igl::readOBJ(obj_file_path_metal, V_metal, F_metal)) {
        std::cerr << "Failed to load metal OBJ file." << std::endl;
        return;
    }
